...
Sentinel 也可以作为 OpenCode 插件工作。它会安装一个轻量插件到 `~/.config/opencode/plugins/claude-code-sentinel.js`，并在 `~/.config/opencode/opencode.json` 中保留已有配置，只补齐 `permission.edit` 和 `permission.bash` 的 `ask` 设置。
...
- `question.asked`：在浮窗中选择答案并回填到 OpenCode
